...
import { ChevronDown, Github, Twitter, FileText, Server, Cpu, HardDrive, Lock, Users, EyeOff, Wallet } from "lucide-react";
        ...
        <div className="flex flex-col lg:flex-row items-center gap-16">
           <div className="lg:w-1/2">
              ...
              <p className="text-lg text-muted-foreground mb-10">
                Power the SanctOS privacy layer by running a node. Earn points, relay encrypted messages, and secure the network.
              </p>
              
              <div className="space-y-4 mb-8">
                 {[
                   { title: "Lite Node", icon: <Cpu />, desc: "Run locally on your machine. (Coming Soon)" },
                   { title: "Home Node", icon: <Server />, desc: "Dedicated Raspberry Pi or mini-PC." },
                   { title: "SanctOS Hardware", icon: <HardDrive />, desc: "Premium dedicated privacy server." },
                 ].map((node, i) => (
                   <div key={i} className="flex items-center gap-4 p-4 rounded-xl bg-white/5 border border-white/10 hover:border-primary/50 transition-colors cursor-pointer group">
              ...
              <WouterLink href="/nodes">
                <Button variant="outline" className="rounded-full border-white/20 hover:bg-white/5 cursor-pointer">
  ...
  return (
    <section className="py-24 relative bg-black/50 border-y border-white/5">
       <div className="container mx-auto px-6 text-center mb-16">
          ...
          <h2 className="text-3xl md:text-5xl font-display font-bold mb-4">Encrypted Group Chats</h2>
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Shared symmetric group keys individually encrypted per device. Zero metadata. Total privacy for your squad.
          </p>
       </div>
          ...
          <div className="relative max-w-3xl mx-auto bg-[#0c0c12] border border-white/10 rounded-2xl p-8 shadow-2xl overflow-hidden">
             ...
             <div className="flex flex-col items-center gap-8">
                ...
                <div className="text-center mt-4">
                   <div className="text-sm font-bold text-white mb-1">Group Key Rotation</div>
                   <div className="text-xs text-muted-foreground">Zero-Knowledge Distribution</div>
                </div>
             </div>
          </div>
       </div>
...
export function Roadmap() {
  const phases = [
    ...
    },
    {
      ...
      items: [
        { title: "Private DeFi Mode", desc: "Private transaction relay, IP/device metadata hiding." },
        { title: "RPC Rotation", desc: "Via Light Nodes." },
        { title: "Light Node Release", desc: "Encrypted persistent local cache, automatic IP rotation." },
        { title: "Private RPC Shield", desc: "Start of Points Program." },
        { title: "Encrypted Group Chats", desc: "Symmetric group keys, key rotation, zero metadata delivery." }
      ]
    },
    {
      phase: "Phase 3",
  ...
  return (
    <section id="roadmap" className="py-32 bg-[#08080c]">
      <div className="container mx-auto px-6">
        <h2 className="text-3xl md:text-5xl font-display font-bold text-center mb-20">Roadmap (2025 Edition)</h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 relative">
           ...
           {phases.map((phase, i) => (
             <div key={i} className="relative group">
                <div className={`w-4 h-4 rounded-full mb-6 relative z-10 shadow-[0_0_10px_currentColor] ${i === 0 ? 'bg-green-400 text-green-400' : 'bg-sanctos-violet text-sanctos-violet'}`} />
                
                <div className="flex items-center justify-between mb-2">
                  <div className="text-xs font-mono text-muted-foreground">{phase.phase}</div>
                  <div className={`text-xs font-bold uppercase tracking-wider ${phase.statusColor}`}>{phase.status}</div>
                </div>
                    ...
                    return (
                      <li key={j} className="text-sm text-muted-foreground flex items-start gap-2">
                        ...
                        <div>
                          {isComplex ? (
                            <>
                              <span className="text-white block mb-0.5">{item.title}</span>
...
export function FAQ() {
  return (
    <section className="py-24 max-w-3xl mx-auto px-6">
      <h2 className="text-3xl font-display font-bold mb-12 text-center">Frequent Questions</h2>
      <Accordion type="single" collapsible className="w-full">
        {[
          { q: "Is SanctOS really private?", a: "Yes. SanctOS uses end-to-end XSalsa20 encryption. We do not collect phone numbers, emails, or IP addresses. All metadata is stripped via our node relay network." },
          { q: "How does encryption work?", a: "Messages are encrypted locally on your device using your wallet's derived keys. Only the recipient can decrypt the message." },
          { q: "Do I need a specific wallet?", a: "SanctOS works with any standard Solana wallet (Phantom, Solflare), but for maximum privacy, we recommend generating a fresh wallet within the SanctOS app." },
          { q: "Does SanctOS store metadata?", a: "No. Our architecture is designed to be metadata-free. We don't know who is talking to whom, or when." }
        ].map((item, i) => (
          <AccordionItem key={i} value={`item-${i}`} className="border-white/10">
            <AccordionTrigger className="text-lg hover:text-primary">{item.q}</AccordionTrigger>
      ...
      <div className="container mx-auto px-6">
        <div className="glass-card max-w-4xl mx-auto p-12 rounded-3xl border border-primary/30 relative overflow-hidden">
           ...
           <div className="relative z-10">
             <h2 className="text-4xl md:text-6xl font-display font-bold mb-6">Take Back Your Privacy</h2>
             <p className="text-xl text-muted-foreground mb-10 max-w-2xl mx-auto">
               Join the thousands of users reclaiming their digital sovereignty on Solana.
             </p>
             <div className="flex flex-col sm:flex-row justify-center gap-6">
                <Button size="lg" className="h-14 px-10 text-lg rounded-full bg-primary hover:bg-primary/90 shadow-[0_0_30px_rgba(167,92,255,0.4)]">
                  Launch Web App
                </Button>
                <Button size="lg" variant="outline" className="h-14 px-10 text-lg rounded-full border-white/20 hover:bg-white/5">
                  Download Node
                </Button>
             </div>
           </div>
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-12">
          ...
          <div className="flex flex-wrap justify-center gap-8 text-sm text-muted-foreground">
            <WouterLink href="/comparison" className="hover:text-white transition-colors cursor-pointer">SanctOS vs. Others</WouterLink>
            <WouterLink href="/nodes" className="hover:text-white transition-colors cursor-pointer">Nodes</WouterLink>
            <WouterLink href="/docs" className="hover:text-white transition-colors cursor-pointer">Docs</WouterLink>
          </div>
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center gap-6 mb-12">
           <Button className="bg-primary hover:bg-primary/90 text-white rounded-full px-8 shadow-[0_0_20px_rgba(167,92,255,0.3)]">
             Launch App
           </Button>
           <WouterLink href="/docs">
             <Button variant="outline" className="rounded-full px-8 border-white/20 hover:bg-white/5 cursor-pointer">
               Read the Docs
             </Button>
           </WouterLink>
        </div>
